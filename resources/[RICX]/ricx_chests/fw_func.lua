...
function RegisterRDMItemUsage(itemname, id)
    if ConfigS.framework == "redemrp" then 
        RegisterServerEvent("RegisterUsableItem:"..itemname, function(source)
            ...
            TriggerEvent("ricx_chests:placing", _source, itemname, id)
        end)
    elseif ConfigS.framework == "redemrp-reboot" then
        RegisterServerEvent("RegisterUsableItem:"..itemname, function(source)
            local _source = source
...
function RedMSQL(dat)
    local a = nil
    local b,c = "SELECT * FROM ricx_chests", {}
    if dat?.identifier then 
        b,c = "SELECT * FROM ricx_chests WHERE identifier=@id2 AND charid=@id3", {id2=dat.identifier, id3=dat.charid}
    end
    if ConfigS.framework == "redemrp" then 
    ...
    elseif ConfigS.framework == "redemrp-reboot" then
        b,c = "SELECT * FROM ricx_chests", {}
        if dat?.identifier then 
            b,c = "SELECT * FROM ricx_chests WHERE identifier=? AND charid=?", {dat.identifier, dat.charid}
        end
...
function RedMSQL_Update(dat, save)
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.execute("UPDATE ricx_chests SET data=@data WHERE identifier=@id2 AND charid=@id3 AND id=@id", {id2 = dat.identifier, id3 = dat.charid, data = save, id = dat.id}, function(done)
        end)
    elseif ConfigS.framework == "redemrp-reboot" then
        MySQL.update("UPDATE ricx_chests SET data=? WHERE identifier=? AND charid=? AND id=?", {save, dat.identifier, dat.charid, dat.id}, function(done)
        end)
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("UPDATE ricx_chests SET data=@data WHERE identifier=@id2 AND charid=@id3 AND id=@id", {id2 = dat.identifier, id3 = dat.charid, data = save, id = dat.id}, function(done)
        end)
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then 
        MySQL.update("UPDATE ricx_chests SET data=@data WHERE identifier=@id2 AND charid=@id3 AND id=@id", {id2 = dat.identifier, id3 = dat.charid, data = save, id = dat.id})
    end
end
--------------------------------------------------------------------------------------------------------------------------------------------
function RedMSQL_Update2(dat, save)
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.execute("UPDATE ricx_chests SET locked=@locked WHERE identifier=@id2 AND charid=@id3 AND id=@id", {id2 = dat.identifier, id3 = dat.charid, locked = save, id = dat.id}, function(done)
        end)
    elseif ConfigS.framework == "redemrp-reboot" then
        MySQL.update("UPDATE ricx_chests SET locked=? WHERE identifier=? AND charid=? AND id=?", {save, dat.identifier, dat.charid, dat.id}, function(done)
        end)
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("UPDATE ricx_chests SET locked=@locked WHERE identifier=@id2 AND charid=@id3 AND id=@id", {id2 = dat.identifier, id3 = dat.charid, locked = save, id = dat.id}, function(done)
        end)
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then 
        MySQL.update("UPDATE ricx_chests SET locked=@locked WHERE identifier=@id2 AND charid=@id3 AND id=@id", {id2 = dat.identifier, id3 = dat.charid, locked = save, id = dat.id})
    end
end
--------------------------------------------------------------------------------------------------------------------------------------------
function RedMSQL_Insert(dat, save)
    local a = nil 
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.execute("INSERT INTO ricx_chests (identifier, charid, locked, data) VALUES (@identifier, @charid, @locked, @data)",  {identifier = dat.identifier, charid = dat.charid, locked = 0, data = save}, function(result)
            Wait(200)
            if result > 0 then 
                a = result
            else
    ...
    elseif ConfigS.framework == "redemrp-reboot" then
        MySQL.insert("INSERT INTO ricx_chests (identifier, charid, locked, data) VALUES (?,?,?,?)",  {dat.identifier, dat.charid, 0, save}, function(result)
            Wait(200)
            if result > 0 then 
                a = result
            else
    ...
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("INSERT INTO ricx_chests (identifier, charid, locked, data) VALUES (@identifier, @charid, @locked, @data)",  {identifier = dat.identifier, charid = dat.charid, locked = 0, data = save}, function(result)
            Wait(200)
    ...
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then 
        MySQL.insert("INSERT INTO ricx_chests (identifier, charid, locked, data) VALUES (@identifier, @charid, @locked, @data)",  {identifier = dat.identifier, charid = dat.charid, locked = 0, data = save}, function(result)
            Wait(200)
            if result > 0 then 
                a = result
            else
...
function RedMSQL_Delete(dat)
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.fetchAll("DELETE FROM ricx_chests WHERE identifier=@identifier AND charid=@charid AND id=@id;", {identifier = dat.identifier, charid = dat.charid, id = dat.id}, function(result)
        end)
    elseif ConfigS.framework == "redemrp-reboot" then 
        MySQL.query("DELETE FROM ricx_chests WHERE identifier=? AND charid=? AND id=?;", {dat.identifier, dat.charid, dat.id}, function(result)
        end)
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("DELETE FROM ricx_chests WHERE identifier=@identifier AND charid=@charid AND id=@id;", {identifier = dat.identifier, charid = dat.charid, id = dat.id}, function(result)
        end)
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then  
        MySQL.query("DELETE FROM ricx_chests WHERE identifier=@identifier AND charid=@charid AND id=@id;", {identifier = dat.identifier, charid = dat.charid, id = dat.id}, function(result)
        end)
    end
end
--------------------------------------------------------------------------------------------------------------------------------------------
local isLockerRegistered = {}
function OpenLocker(src, id, chest)
    local _source = tonumber(src) 
    local stashid = tostring(id)
    local size = ConfigS.Chests[chest].stash_size[ConfigS.framework]
    if ConfigS.framework == "redemrp" then 
    ...
    elseif ConfigS.framework == "vorp" then 
        if not isLockerRegistered[stashid] then
            VorpInv.registerInventory(stashid, "Locker Chest", size, true, true, true)
            isLockerRegistered[stashid] = true
        end
        VorpInv.OpenInv(_source, stashid)
    elseif ConfigS.framework == "rsg" then 
        TriggerClientEvent("ricx_chests:rsg_stash", _source, stashid, size)
        --exports["rsg-inventory"]:OpenInventory(_source, stashid, size)
    elseif ConfigS.framework == "qbr" then 
        TriggerClientEvent("ricx_chests:qbr_stash", _source, stashid, size)
    elseif ConfigS.framework == "qbr2" then 
        TriggerClientEvent("ricx_chests:qbr2_stash", _source, stashid, size)
    end
end
--------------------------------------------------------------------------------------------------------------------------------------------
