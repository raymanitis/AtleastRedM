...
function RedMSQL(dat)
    local a = nil
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.fetchAll("SELECT * FROM ricx_bisonhunt WHERE identifier=@id2 AND charid=@id3", {id2=dat.identifier, id3=dat.charid}, function(result)
            Wait(200)
            if result[1] then
                a = result
            else
    ...
    elseif ConfigS.framework == "redemrp-reboot" then
        MySQL.query("SELECT * FROM ricx_bisonhunt WHERE identifier=? AND charid=?", {dat.identifier, dat.charid}, function(result)
            Wait(200)
            if result[1] then
                a = result
            else
    ...
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("SELECT * FROM ricx_bisonhunt WHERE identifier=@id2 AND charid=@id3", {id2=dat.identifier, id3=dat.charid}, function(result)
            Wait(200)
            if result[1] then
                a = result
            else
    ...
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then 
        MySQL.query("SELECT * FROM ricx_bisonhunt WHERE identifier=@id2 AND charid=@id3", {id2=dat.identifier, id3=dat.charid}, function(result)
            Wait(200)
            if result[1] then
                a = result
            else
...
function RedMSQL_Update(dat, save)
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.execute("UPDATE ricx_bisonhunt SET data=@data WHERE identifier=@id2 AND charid=@id3", {id2 = dat.identifier, id3 = dat.charid, data = save}, function(done)
        end)
    elseif ConfigS.framework == "redemrp-reboot" then
        MySQL.update("UPDATE ricx_bisonhunt SET data=? WHERE identifier=? AND charid=?", {save, dat.identifier, dat.charid}, function(done)
        end)
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("UPDATE ricx_bisonhunt SET data=@data WHERE identifier=@id2 AND charid=@id3", {id2 = dat.identifier, id3 = dat.charid, data = save}, function(done)
        end)
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then 
        MySQL.update("UPDATE ricx_bisonhunt SET data=@data WHERE identifier=@id2 AND charid=@id3", {id2 = dat.identifier, id3 = dat.charid, data = save})
    end
end
--------------------------------------------------------------------------------------------------------------------------------------------
function RedMSQL_Insert(dat, save)
    local a = nil 
    if ConfigS.framework == "redemrp" then 
        MySQL.Async.execute("INSERT INTO ricx_bisonhunt (identifier, charid, data) VALUES (@identifier, @charid, @data)",  {identifier = dat.identifier, charid = dat.charid, data = save}, function(result)
            Wait(200)
            if result > 0 then 
                a = result
            else
    ...
    elseif ConfigS.framework == "redemrp-reboot" then
        MySQL.insert("INSERT INTO ricx_bisonhunt (identifier, charid, data) VALUES (?,?,?)",  {dat.identifier, dat.charid, save}, function(result)
            Wait(200)
            if result > 0 then 
                a = result
            else
    ...
    elseif ConfigS.framework == "vorp" then 
        exports.ghmattimysql:execute("INSERT INTO ricx_bisonhunt (identifier, charid, data) VALUES (@identifier, @charid, @data)",  {identifier = dat.identifier, charid = dat.charid, data = save}, function(result)
            Wait(200)
    ...
    elseif ConfigS.framework == "qbr" or ConfigS.framework == "qbr2" or ConfigS.framework == "rsg" then 
        MySQL.insert("INSERT INTO ricx_bisonhunt (identifier, charid, data) VALUES (@identifier, @charid, @data)",  {identifier = dat.identifier, charid = dat.charid, data = save}, function(result)
            Wait(200)
            if result > 0 then 
                a = result
            else
